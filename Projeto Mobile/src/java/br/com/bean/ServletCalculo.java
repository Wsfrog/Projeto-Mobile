    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            /* TODO output your page here. You may use following sample code. */
               ...
               int horas = Integer.valueOf(request.getParameter("horas"));
            c.setSalario(salario);
            c.setHoras(horas);
            double salarioReceber = 0;
            
            if ( horas > 160){
                salarioReceber = c.salarioHoraExtra();        
            } else {              
                salarioReceber = c.salarioBase();
            }
            request.setAttribute("salarioReceber",salarioReceber);
